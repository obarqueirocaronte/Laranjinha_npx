...
export const UserZone: React.FC<UserZoneProps> = ({ onClose }) => {
    ...
    const InviteView = () => (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col lg:flex-row gap-8 h-full">
            {/* Form Side */}
            <div className="flex-1 bg-gradient-soft border border-orange-200/50 shadow-glass p-8 rounded-[32px] flex flex-col backdrop-blur-md">
                ...
                <div className="space-y-6 flex-1">
                    <div>
                        ...
                        <input type="text" className="w-full px-4 py-3 rounded-2xl bg-white/40 border border-orange-200/50 shadow-glass focus:border-orange-400 focus:ring-0 transition-all font-[Comfortaa] font-bold" placeholder="ex: João Silva" value={newItem.name} onChange={e => setNewItem({ ...newItem, name: e.target.value })} />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">E-mail Corporativo</label>
                        <input type="email" className="w-full px-4 py-3 rounded-2xl bg-white/40 border border-orange-200/50 shadow-glass focus:border-orange-400 focus:ring-0 transition-all font-[Comfortaa] font-bold" placeholder="ex: joao@empresa.com" value={newItem.email} onChange={e => setNewItem({ ...newItem, email: e.target.value })} />
                    </div>
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {(Object.entries(roles) as [Role, typeof roles['sdr']][]).map(([key, config]) => (
                                <div key={key} onClick={() => setNewItem({ ...newItem, role: key })} className={cn("cursor-pointer p-4 rounded-xl border-2 transition-all flex flex-col items-center gap-2", newItem.role === key ? "border-orange-500 bg-gradient-to-br from-orange-400 to-orange-500 text-white shadow-orange-500/20 shadow-md" : "border-orange-200/50 bg-gradient-soft shadow-glass text-slate-600 hover:border-orange-300")}>
                                    <config.icon size={20} />
            ...
            <div className="flex-1 bg-gradient-to-br from-orange-50/40 to-transparent p-8 rounded-[32px] border border-orange-200/50 flex flex-col backdrop-blur-sm">
                ...
                <div className="flex-1 bg-gradient-soft border border-orange-200/50 shadow-glass rounded-2xl shadow-sm p-8 font-sans overflow-y-auto">
                    ...
                    <div className="space-y-4 text-slate-600 leading-relaxed text-sm">
                        ...
                        <p>Você foi convidado para o workspace do <strong>Pipeline de Vendas</strong> como {roles[newItem.role].label}.</p>
                        ...
                        <div className="py-4">
                            <button className="bg-blue-600 text-white px-6 py-3 rounded-lg font-bold mx-auto block">Completar Cadastro</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const SuccessView = () => (
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex flex-col items-center justify-center h-full max-w-lg mx-auto text-center">
            <div className="w-24 h-24 bg-emerald-100 rounded-full flex items-center justify-center mb-6 text-emerald-600"><CheckCircle2 size={48} /></div>
            ...
            <button onClick={() => setView('list')} className="px-8 py-3 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 transition-all">Voltar à Lista</button>
        </motion.div>
    );
    ...
    const ProfileView = () => {
        ...
        const toggleIntegration = (type: keyof typeof integrations) => {
            setIntegrations(prev => ({ ...prev, [type]: { ...prev[type], enabled: !prev[type].enabled } }));
        };

        const updateConfig = (type: keyof typeof integrations, field: string, value: string) => {
            setIntegrations(prev => ({ ...prev, [type]: { ...prev[type], [field]: value } }));
        };
        ...
        const handleSave = async () => {
            ...
            try {
                ...
                // update local state
                const updatedUsers = users.map(u => u.id === selectedUser.id ? { ...u, integrations } : u);
                setUsers(updatedUsers);
                setSelectedUser({ ...selectedUser, integrations }); // update selected user view
        ...
        return (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col h-full gap-6 pb-6">
                {/* Profile Header */}
                <div className="bg-gradient-soft border border-orange-200/50 shadow-glass p-6 rounded-[32px] flex items-center justify-between backdrop-blur-md">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            ...
                            <span className={cn("px-3 py-1 rounded-lg text-[10px] font-black uppercase border tracking-wider", (selectedUser.role && roles[selectedUser.role]) ? roles[selectedUser.role].color : roles.sdr.color)}>
                                {(selectedUser.role && roles[selectedUser.role]) ? roles[selectedUser.role].label : 'SDR'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button onClick={handleSave} disabled={isSaving} className="px-6 py-2.5 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1">
                    {/* E-mail Config */}
                    <div className={cn("bg-white border shadow-glass rounded-[32px] p-6 flex flex-col transition-all", integrations.email.enabled ? "border-orange-300 ring-2 ring-orange-100" : "border-slate-300 hover:border-slate-400")}>
                        ...
                        {integrations.email.enabled && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 flex-1">
                                <div><label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">Servidor SMTP</label>
                                    <input type="text" placeholder="smtp.gmail.com" value={integrations.email.host || ''} onChange={(e) => updateConfig('email', 'host', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-orange-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" /></div>
                                <div><label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">Porta</label>
                                    <input type="text" placeholder="587" value={integrations.email.port || ''} onChange={(e) => updateConfig('email', 'port', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-orange-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" /></div>
                                <div><label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">E-mail Usuário</label>
                                    <input type="email" placeholder="usuario@empresa.com" value={integrations.email.user || ''} onChange={(e) => updateConfig('email', 'user', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-orange-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" /></div>
                                <div><label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">Senha / App Password</label>
                                    <input type="password" placeholder="••••••••" value={integrations.email.pass || ''} onChange={(e) => updateConfig('email', 'pass', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-orange-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" /></div>
                            </motion.div>
                        )}
                    </div>

                    {/* Voice Config */}
                    <div className={cn("bg-white border shadow-glass rounded-[32px] p-6 flex flex-col transition-all", integrations.voice.enabled ? "border-sky-300 ring-2 ring-sky-100" : "border-slate-300 hover:border-slate-400")}>
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                        ...
                        {integrations.voice.enabled && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 flex-1">
                                <div><label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">Servidor SIP</label>
                                    <input type="text" placeholder="tip3.npxtech.com.br" value={integrations.voice.sipServer || ''} onChange={(e) => updateConfig('voice', 'sipServer', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-sky-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" /></div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">Ramal</label>
                                    <input type="text" placeholder="11012" value={integrations.voice.extension || ''} onChange={(e) => updateConfig('voice', 'extension', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-sky-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">SenhaSIP</label>
                                    <input type="password" placeholder="••••••••" value={integrations.voice.password || ''} onChange={(e) => updateConfig('voice', 'password', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-sky-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" />
                                </div>
                                <div className="pt-2">
                                    {integrations.voice.extension && integrations.voice.password ? (
                                        <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-xl text-emerald-600">
                                            <CheckCircle2 size={16} />
                                            <span className="text-xs font-black uppercase tracking-tight">Ramal Configurado & OK</span>
                                        </div>
                                    ) : (
                                        <button className="w-full py-2 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-xl text-xs font-bold transition-all uppercase tracking-widest border border-slate-200">
                    ...
                    {/* Aurora Chat Config */}
                    <div className={cn("bg-white border shadow-glass rounded-[32px] p-6 flex flex-col transition-all", integrations.aurora.enabled ? "border-violet-300 ring-2 ring-violet-100" : "border-slate-300 hover:border-slate-400")}>
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="font-bold text-slate-800 text-lg">Aurora Chat</h3>
                            </div>
                        ...
                        {integrations.aurora.enabled && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 flex-1">
                                <div><label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">ID do Usuário Aurora</label>
                                    <input type="text" placeholder="Ex: 2244" value={integrations.aurora.auroraUserId || ''} onChange={(e) => updateConfig('aurora', 'auroraUserId', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-violet-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" /></div>
                                <div><label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wider">Número WhatsApp</label>
                                    <input type="text" placeholder="+55 11 99999-9999" value={integrations.aurora.phoneNumber || ''} onChange={(e) => updateConfig('aurora', 'phoneNumber', e.target.value)} className="w-full px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 focus:border-violet-400 focus:ring-0 text-sm font-bold text-slate-700 transition-colors" /></div>
                            </motion.div>
                        )}
                    </div>
                </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col h-full">
            <div className="flex-1 bg-white border border-orange-200/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl overflow-hidden flex flex-col">
                <div className="p-6 border-b border-orange-100/60 flex items-center justify-between bg-gradient-to-b from-orange-50/60 to-transparent">
                    ...
                    <div className="flex items-center gap-2">
                        <button className="p-2 text-slate-600 hover:text-slate-600 hover:bg-slate-50 border border-slate-200 shadow-sm rounded-lg transition-all"><Search size={18} /></button>
                        <button className="p-2 text-slate-600 hover:text-slate-600 hover:bg-slate-50 border border-slate-200 shadow-sm rounded-lg transition-all"><Filter size={18} /></button>
                    </div>
                </div>
                ...
                <div className="overflow-x-auto">
                    {isLoading ? (
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-slate-50/50 border-b border-orange-100/60">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-black text-slate-600 uppercase tracking-widest text-right">Integrações / Ações</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                                {users.map(user => (
                                    <tr key={user.id} className="hover:bg-slate-50 transition-colors group cursor-pointer" onClick={() => { setSelectedUser(user); setView('profile'); }} style={{ fontFamily: 'Comfortaa, cursive' }}>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center text-orange-600 font-black text-sm border border-orange-200 shadow-sm">
                                                    {user.name ? user.name.charAt(0).toUpperCase() : '?'}
                                                </div>
                                                <div>
                                                    <div className="font-bold text-slate-900 text-sm">{user.name || 'Sem Nome'}</div>
                                                    <div className="text-xs text-slate-600 font-medium">{user.email}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className={cn("px-3 py-1 rounded-lg text-[10px] font-black uppercase border tracking-wider", (user.role && roles[user.role]) ? roles[user.role].color : roles.sdr.color)}>
                                                {(user.role && roles[user.role]) ? roles[user.role].label : 'SDR'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center justify-end gap-3">
                                                <div className="flex items-center gap-1.5 opacity-90">
                                                    {user.integrations?.email?.enabled && <Mail size={14} className="text-orange-500" />}
                                                    {user.integrations?.voice?.enabled && <Phone size={14} className="text-sky-500" />}
                                                    {user.integrations?.aurora?.enabled && <MessageSquare size={14} className="text-violet-500" />}
                                                </div>
                                                <button onClick={(e) => { e.stopPropagation(); setSelectedUser(user); setView('profile'); }} className="p-2 text-slate-600 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-all opacity-0 group-hover:opacity-100 border border-transparent group-hover:border-orange-200">
                                                    <Settings size={16} />
                                                </button>
                                            </div>
                                        </td>
            ...
            <div className="w-full max-w-7xl h-full flex flex-col p-6">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    <AnimatePresence mode="wait">
                        ...
                        {view === 'invite' && <InviteView key="invite" />}
