...
import { X, Clock, CalendarDays, ChevronLeft, ChevronRight, ListFilter, Search } from 'lucide-react';
...
export const SchedulePreviewModal: React.FC<SchedulePreviewModalProps> = ({
    ...
    const [filterType, setFilterType] = useState<'all' | 'manual' | 'cadence'>('all');
    const [searchQuery, setSearchQuery] = useState('');
    const [showOnlyInProgress, setShowOnlyInProgress] = useState(false);

    const scheduledLeads = useMemo(() => {
        let base = leads.filter(l => {
            const hasDirectSchedule = Boolean(l.metadata?.next_contact_at);
            const hasCadenceReturn = Boolean(l.lead_cadence_id && l.cadence_status === 'active');
            return hasDirectSchedule || hasCadenceReturn;
        });

        if (filterType === 'manual') {
            base = base.filter(l => l.metadata?.next_contact_type !== 'cadence');
        } else if (filterType === 'cadence') {
            base = base.filter(l => l.metadata?.next_contact_type === 'cadence' || l.lead_cadence_id);
        }

        if (showOnlyInProgress) {
            base = base.filter(l => l.cadence_status === 'active' || l.qualification_status === 'qualified');
        }
        ...
        return base.sort((a, b) => {
            const dateA = a.metadata?.next_contact_at ? new Date(a.metadata.next_contact_at).getTime() : Infinity;
            const dateB = b.metadata?.next_contact_at ? new Date(b.metadata.next_contact_at).getTime() : Infinity;
            return dateA - dateB;
        });
    }, [leads, filterType, showOnlyInProgress, searchQuery]);
                ...
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 lg:p-10">
                    ...
                    >
                        ...
                        <div className="w-full md:w-[320px] bg-white border-b md:border-b-0 md:border-r border-slate-100 p-6 flex flex-col shrink-0 relative overflow-hidden">
                            ...
                            </div>                            {/* Sidebar Stats & Filters */}
                            <div className="mt-6 flex flex-col gap-4 relative z-10">
                                <div className="space-y-2">
                                    <h4 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">Filtros</h4>
                                    <div className="relative group">
                                        ...
                                        <input 
                                            ...
                                            className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-9 pr-3 py-2 text-[11px] font-bold text-slate-600 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500/10 focus:border-orange-200 transition-all shadow-inner"
                                        />
                                    </div>
                                    <div className="flex bg-slate-100/50 p-1 rounded-xl border border-slate-100">
                                        {(['all', 'manual', 'cadence'] as const).map(t => (
                                            <button 
                                                key={t}
                                                onClick={() => setFilterType(t)}
                                                className={cn(
                                                    "flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all",
                                                    filterType === t ? "bg-white text-orange-600 shadow-sm border border-slate-200/50" : "text-slate-500 hover:text-slate-700"
                                                )}
                                            >
                                                {t === 'all' ? 'Ver Tudo' : t === 'manual' ? 'Manuais' : 'Cadência'}
                                            </button>
                                        ))}
                                    </div>
                                    <label className="flex items-center gap-2 px-1 cursor-pointer group">
                                        <input 
                                            ...
                                            className="w-3.5 h-3.5 rounded text-orange-500 focus:ring-orange-500 border-slate-300 transition-all"
                                        />
                                        <span className="text-[10px] font-bold text-slate-500 group-hover:text-slate-700 transition-colors">Somente em Andamento</span>
                                    </label>
                                </div>
                                ...
                                <div className="space-y-3">
                                    {selectedDay !== null && (
                                        <button
                                            ...
                                            className="w-full flex items-center gap-2 text-[10px] font-black text-orange-600 uppercase tracking-widest py-2 px-3 bg-orange-50 rounded-xl border border-orange-100 hover:bg-orange-100 transition-colors shadow-sm"
                                        >
                                            <ListFilter size={12} />
                                            Limpar dia ({scheduledLeads.length})
                                        </button>
                                    )}
                                    <div className="group p-4 bg-orange-50/50 rounded-2xl border border-orange-100/50 hover:bg-orange-50 transition-all">
                                        ...
                                        <p className="text-[9px] font-bold text-orange-500 uppercase tracking-tighter opacity-70">Agendamentos diretos do SDR</p>
                                    </div>
                                    <div className="group p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100/50 hover:bg-indigo-50 transition-all">
                                        <div className="flex items-center justify-between mb-1">
                                            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">Cadências</span>
                                            <span className="text-sm font-black text-indigo-700">{scheduledLeads.filter(l => l.metadata?.next_contact_type === 'cadence' || l.lead_cadence_id).length}</span>
                                        </div>
                                        <p className="text-[9px] font-bold text-indigo-500 uppercase tracking-tighter opacity-70">Fluxos automáticos de retorno</p>
                                    </div>
                                </div>
                            </div>
                        </div>
