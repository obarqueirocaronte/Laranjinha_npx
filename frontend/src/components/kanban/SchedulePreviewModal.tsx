...
export const SchedulePreviewModal: React.FC<SchedulePreviewModalProps> = ({
    ...
    const isDateScheduled = (day: number) => {
        ...
        return scheduledLeads.some(l => new Date(l.metadata!.next_contact_at!).toDateString() === dateString);
    };
                                ...
                                <div className="grid grid-cols-7 gap-1">
                                    ...
                                    {daysArray.map(day => {
                                        ...
                                        return (
                                            <div
                                                key={day}
                                                className={`h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-all cursor-default ${today ? 'bg-indigo-50 text-indigo-600 ring-1 ring-indigo-200 shadow-sm' :
                                                    scheduled ? 'bg-orange-50 text-orange-600 ring-1 ring-orange-200 shadow-sm' :
                            ...
                            <div className="px-8 py-6 pb-4 flex justify-between items-center shrink-0 border-b border-slate-100 relative z-10 bg-white/50 backdrop-blur-md">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-0.5" style={{ fontFamily: 'Quicksand, sans-serif' }}>
                                            Leads retornando hoje e no futuro
                                        </p>
                                    </div>
                                </div>
                                <button
                                ...
                                {scheduledLeads.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[12px] font-bold text-slate-400 mt-1 max-w-[200px]" style={{ fontFamily: 'Quicksand, sans-serif' }}>Sua agenda está livre no momento.</p>
                                        </div>
                                    </div>
                                ) : (
                                        ...
                                        {scheduledLeads.map(l => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    onClick={() => {
                                                        if (onLeadClick) onLeadClick(l);
                                                    }}
                                                >
                                                    <div className={`absolute left-0 top-0 bottom-0 w-1 ${isPastCard ? 'bg-red-400' : isTodayCard ? 'bg-orange-400' : 'bg-indigo-400'}`} />

                                                    <div className="flex justify-between items-start mb-3">
                                                        <span className="text-sm font-black text-slate-800 truncate" style={{ fontFamily: 'Comfortaa, cursive' }}>{l.full_name}</span>
                                                        <span className={`shrink-0 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${isPastCard ? 'bg-red-50 text-red-600' : isTodayCard ? 'bg-orange-50 text-orange-600' : 'bg-indigo-50 text-indigo-600'}`}>
                                                    ...
                                                    <div className="flex items-center gap-2 mb-3">
                                                        ...
                                                        {l.company_name && (
                                                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-none" style={{ fontFamily: 'Quicksand, sans-serif' }}>
                                                                {l.company_name}
                                                            </span>
                                                        )}
                                                    </div>
